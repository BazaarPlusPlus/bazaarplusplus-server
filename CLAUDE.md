...
- Bundle-final battle metadata is not part of the current V4 mod-facing wire contract. The `battles.is_final_battle` column is intentionally retained as an internal placeholder — it stays in the schema (`migrations/0001_v4_initial.sql`) but is never written by `BATTLE_INSERT_SQL` nor read by the ghost-battle query, and must not be exposed in the wire (`test/ghostBattles.query.test.ts` asserts its absence). Do not reintroduce bundle-final metadata, or drop the column, without updating `docs/api-reference.md`, server tests, and mod client tests together.
